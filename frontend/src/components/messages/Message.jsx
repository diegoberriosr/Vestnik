...
const Message = ({ message, handleOpenDeleteModal, isGroupChat, numberOfPartners}) => {
  ...
  return (
    <li className={`mt-5 max-w-6/12 flex ${message.sender.id === user.id ?  'flex-row-reverse' : 'justify-start'} space-x-3.5`}>
        ...
        <div className={`sw-full space-y-2 flex flex-col ${ message.sender.id === user.id ? 'items-end px-3.5' : 'items-start' }`}>
        ...
        <div className={`relative max-w-[70%] ${ message.sender.id === user.id ? 'bg-sky-500 text-white' : 'bg-gray-100'} rounded-full rounded-full p-2.5 inline-block cursor-pointer`} onClick={ () => handleOpenDeleteModal(message)}>
          { message.content}
        </div>
